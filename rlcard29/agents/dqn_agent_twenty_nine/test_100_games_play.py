...
if __name__ == '__main__':
    ...
    print_header("Match Simulation Over!")
    print(f"Total Wins - Team 0 (DQN Agent): {team_0} | Team 1 (Random Agents): {team_1}")

    plt.figure(figsize=(8, 5))
    agents = ['DQN Agent (Team 0)', 'Random Agents (Team 1)']
    wins = [team_0, team_1]
    plt.bar(agents, wins, color=['blue', 'orange'])
    plt.xlabel('Agent Type')
    plt.ylabel('Number of Wins')
    plt.title('Win Distribution Over 100 Games')
    plt.ylim(0, 100)  
    plt.savefig(os.path.join("rlcard29/plots", f'wins_bar_chart_{timestamp}.png'))
    plt.close()

    os.makedirs("rlcard29/plots", exist_ok=True)
    print(f"Win distribution plot saved to rlcard29/plots/wins_bar_chart_{timestamp}.png")